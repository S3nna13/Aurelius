#!/usr/bin/env python3
"""Train Aurelius on Aurelius_Training_Data.jsonl (17.7M tokens)."""
...
logger = logging.getLogger("train_jsonl")

CONFIG = AureliusConfig(
    d_model=768, n_layers=10, n_heads=12, n_kv_heads=4, head_dim=64,
    d_ff=2048, vocab_size=8192, max_seq_len=512, rope_theta=500_000.0,
    rms_norm_eps=1.0e-6, dropout=0.0, tie_embeddings=True,
)
...
def train(model, train_loader, val_loader, device, total_steps=2000, warmup=100,
          lr=3e-4, min_lr=3e-5, wd=0.1, max_grad_norm=1.0, log_interval=50,
          val_interval=500, save_dir=Path("checkpoints/aurelius-jsonl")):
    model.to(device)
    model.train()
    ...
    opt = AdamW([{"params": decay, "weight_decay": wd}, {"params": no_decay, "weight_decay": 0.0}],
    ...
    while global_step < total_steps:
        ...
        if global_step % log_interval == 0:
            ...
            logger.info("Step %4d/%d | loss=%.4f | lr=%.2e | tok=%s | tps=%.0f",
                        global_step, total_steps, loss.item(), lr_now,
                        f"{tokens_seen:,}", tokens_seen / elapsed)
            sys.stdout.flush()

        # Validation disabled — MPS hangs on train/eval mode switch
        ...
        #     logger.info(">>> VAL @ step %d | loss=%.4f | ppl=%.2f", global_step, avg_val, math.exp(avg_val))
        #     model.train()
    ...
    torch.save({"step": global_step, "tokens_seen": tokens_seen,
                ...
                "scheduler_state_dict": sched.state_dict()}, ckpt)
    logger.info("Saved checkpoint: %s", ckpt)

    # Final validation disabled — MPS hang risk
    avg_val = None
    logger.info("Training complete. Skipping validation on MPS.")

    report = {
        "model_params": n_params, "total_steps": global_step,
        "tokens_seen": tokens_seen, "total_time_sec": time.monotonic() - start,
        "final_train_loss": history[-1]["loss"] if history else None,
        "val_loss": avg_val, "val_perplexity": None, "history": history,
    }
...
def main():
    ...
    train_ds = ShardDataset(["data/pretrain/jsonl/train_0.npy"], CONFIG.max_seq_len, stride=128)
    val_ds = ShardDataset(["data/pretrain/jsonl/val_0.npy"], CONFIG.max_seq_len, stride=128)
    ...
    report = train(model, train_loader, val_loader, device, total_steps=1000,
                   save_dir=Path("checkpoints/aurelius-jsonl"))

    logger.info("=" * 50)
    logger.info("JSONL CORPUS TRAINING COMPLETE")
