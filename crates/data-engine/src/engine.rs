...
impl DataEngineInner {
    pub fn new() -> Self {
        ...
        skills.push_back(InternalSkill { id: "code-review".into(), name: "Code Review".into(), description: "Automated code review.".into(), active: true, version: "1.0.0".into(), category: "builtin".into(), risk_score: 0.1, allow_level: "auto".into(), instructions: "Review code for quality.".into() });
        skills.push_back(InternalSkill { id: "refactor".into(), name: "Refactor Assistant".into(), description: "Suggest safe refactors.".into(), active: true, version: "1.0.0".into(), category: "builtin".into(), risk_score: 0.2, allow_level: "review".into(), instructions: "Suggest refactors.".into() });
        skills.push_back(InternalSkill { id: "test-gen".into(), name: "Test Generator".into(), description: "Generate unit tests.".into(), active: false, version: "0.9.0".into(), category: "builtin".into(), risk_score: 0.3, allow_level: "review".into(), instructions: "Generate tests.".into() });

        let mut workflows = VecDeque::new();
        workflows.push_back(InternalWorkflow { id: "daily-backup".into(), name: "Daily Backup".into(), status: "completed".into(), last_run: 0.0, duration: 42.0, event_count: 3 });
        workflows.push_back(InternalWorkflow { id: "data-ingest".into(), name: "Data Ingest".into(), status: "failed".into(), last_run: 0.0, duration: 0.0, event_count: 2 });
        workflows.push_back(InternalWorkflow { id: "health-check".into(), name: "Health Check".into(), status: "running".into(), last_run: 0.0, duration: 12.0, event_count: 1 });

        let mut models = VecDeque::new();
        models.push_back(InternalModelInfo { id: "aurelius-v1".into(), name: "Aurelius v1".into(), description: "1.3B base model".into(), path: "/models/aurelius-v1".into(), parameter_count: 1_300_000_000, state: "unloaded".into(), loaded_at: None });
        models.push_back(InternalModelInfo { id: "aurelius-chat".into(), name: "Aurelius Chat".into(), description: "1.3B chat-tuned".into(), path: "/models/aurelius-chat".into(), parameter_count: 1_300_000_000, state: "unloaded".into(), loaded_at: None });
        models.push_back(InternalModelInfo { id: "aurelius-code".into(), name: "Aurelius Code".into(), description: "1.3B code model".into(), path: "/models/aurelius-code".into(), parameter_count: 1_300_000_000, state: "unloaded".into(), loaded_at: None });
        ...
        for step in 0..50 {
            ...
            val_loss = f64::max(val_loss - 0.08 * (1.0 - val_loss / 4.2), 1.5) + (Self::rand_simple(step * 3) * 0.05 - 0.025);
            points.push(InternalDataPoint {
                step: (step + 1) as u32,
                train_loss: ((train_loss * 100.0) as f64).round() / 100.0,
                val_loss: ((val_loss * 100.0) as f64).round() / 100.0,
                learning_rate: if step < 10 { 1e-4 + step as f64 * 9e-5 } else { 1e-3 - (step as f64 - 10.0) * 2e-5 },
        ...
        training_runs.push(InternalTrainingRun {
            id: "run-v1-001".into(), name: "Aurelius v1 Pre-training".into(), model_id: "aurelius-v1".into(),
            status: "completed".into(), started_at: start_ts, current_epoch: 3, total_epochs: 3,
            best_val_loss: 1.52, current_lr: 0.0, total_steps: 50, data_points: points,
        });
        ...
        for step in 0..40 {
            ...
            val_loss2 = f64::max(val_loss2 - 0.06 * (1.0 - val_loss2 / 3.8), 1.8) + (Self::rand_simple(step * 3 + 100) * 0.04 - 0.02);
            points2.push(InternalDataPoint {
                step: (step + 1) as u32,
                train_loss: ((train_loss * 100.0) as f64).round() / 100.0,
                val_loss: ((val_loss2 * 100.0) as f64).round() / 100.0,
                learning_rate: if step < 8 { 2e-5 + step as f64 * 2.5e-5 } else { 2e-4 - (step as f64 - 8.0) * 5e-6 },
                accuracy: if step > 3 { f64::min((step as f64 - 3.0) * 2.2 + 5.0, 88.0) } else { 0.0 },
                grad_norm: Self::rand_simple(step * 7 + 100) * 1.5 + 0.8,
            });
        }
        training_runs.push(InternalTrainingRun {
            id: "run-chat-002".into(), name: "Aurelius Chat SFT".into(), model_id: "aurelius-chat".into(),
            status: "running".into(), started_at: start_ts + 43200.0, current_epoch: 2, total_epochs: 5,
            best_val_loss: 1.89, current_lr: 1.5e-4, total_steps: 40, data_points: points2,
        });
        ...
        for step in 0..60 {
            ...
            val_loss3 = f64::max(val_loss3 - 0.1 * (1.0 - val_loss3 / 4.5), 1.2) + (Self::rand_simple(step * 3 + 200) * 0.06 - 0.03);
        ...
        training_runs.push(InternalTrainingRun {
            id: "run-code-003".into(), name: "Aurelius Code Fine-tune".into(), model_id: "aurelius-code".into(),
            status: "queued".into(), started_at: 0.0, current_epoch: 0, total_epochs: 10,
            best_val_loss: 0.0, current_lr: 0.0, total_steps: 60, data_points: points3,
        });
