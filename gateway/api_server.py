...
class AureliusRequestHandler(BaseHTTPRequestHandler):
    ...
    def _stream_response(
        self, content: str, prompt_tokens: int, model: str, start_time: float
    ) -> None:
        self._send_stream_header()
        chunk_size = max(1, len(content) // 20) if len(content) > 20 else 1
        pos = 0
        completion_tokens = 0
        ...
        self.wfile.flush()
        self._record_metrics(start_time, 200)
    ...
    def do_POST(self):
        ...
        if request.stream:
            self._stream_response(content, prompt_tokens, request.model, start)
        else:
...
if __name__ == "__main__":
    ...
    from .engine_loader import build_engine
    ...
    parser.add_argument(
        "--engine",
        choices=["vllm", "agentic", "mock"],
        default="mock",
        help="Inference engine backend (default: mock)",
    )
    parser.add_argument(
        "--model-path",
        default=os.environ.get("AURELIUS_MODEL_PATH", ""),
        help="Model path or HuggingFace repo ID (default: AURELIUS_MODEL_PATH env var or empty)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--quantization",
        choices=["fp8", "int8", "awq", "gptq", "none"],
        default="none",
        help="Quantization method for vLLM (default: none)",
    )
    parser.add_argument(
        "--speculative-decoding",
        action="store_true",
        help="Enable speculative decoding in vLLM",
    )
    parser.add_argument(
        "--n-spec-tokens",
        type=int,
        default=5,
        help="Number of speculative tokens for speculative decoding (default: 5)",
    )
    ...
    generate_fn, backend_label = build_engine(
        ...
        n_spec_tokens=args.n_spec_tokens,
    )
    server = create_server(args.host, args.port, generate_fn, auth_middleware=auth_mw)
    server._is_mock = args.engine == "mock"
    server._started_at = time.time()
    logger.info(
        "Aurelius API server listening on http://%s:%d (engine=%s)",
        args.host,
        args.port,
        backend_label,
    )
