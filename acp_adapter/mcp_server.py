...
class StdioMCPServer(MCPServer):
    ...
    def _handle_tools_list(self, params: dict) -> dict:
        ...
        try:
            ...
            for name in names:
                spec = (
                    self._tool_registry.get_spec(name)
                    if hasattr(self._tool_registry, "get_spec")
                    else None
                )
                if spec and hasattr(spec, "parameters"):
                    input_schema = {
                        "type": "object",
                        "properties": {},
                        "required": list(getattr(spec, "required", [])),
                    }
    ...
    def _handle_tools_call(self, params: dict) -> dict:
        ...
        if self._tool_registry is None:
            return {"content": [{"type": "text", "text": "Tool registry not configured"}]}
        try:
            if hasattr(self._tool_registry, "invoke"):
                result = self._tool_registry.invoke(name, **arguments)
                output = result.output if hasattr(result, "output") else str(result)
                return {"content": [{"type": "text", "text": str(output)}]}
            return {"content": [{"type": "text", "text": f"Tool {name} not invocable"}]}
