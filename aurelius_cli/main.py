...
Usage:
    ...
    aurelius chat --react       # enable the ReAct loop
    aurelius serve              # start API server + web UI
    aurelius serve --engine agentic  # ReAct backend
...
def _load_generate_fn(
    ...
    *,
    ...
    if react:
        from src.serving.agentic_runtime import ByteTokenizer, build_agentic_prompt_generate_fn

        tokenizer = _tokenizer or ByteTokenizer()
        return build_agentic_prompt_generate_fn(
            model,
            tokenizer,
                ...
                if arg:
                    try:
                        ...
                        for p in BUILTIN_PERSONAS:
                            ureg.register(p)
                        persona = ureg.get(arg)
                        system_prompt = persona.system_prompt
                        history.clear()
                        if _RICH and _console:
                            _console.print(
                                f"[dim]  Switched to persona '{arg}'. History cleared.[/dim]\n"
                            )
                        else:
                            print(f"  Switched to persona '{arg}'. History cleared.\n")
                    except Exception as exc:
                        if _RICH and _console:
                            _console.print(
                                f"[aurelius.warn]  Unknown persona: {arg} ({exc})[/aurelius.warn]\n"
                            )
                        else:
                            print(f"  Unknown persona: {arg} ({exc})\n")
                else:
                    if _RICH and _console:
                        _console.print(f"  [dim]Current: {system_prompt[:80]}...[/dim]\n")
                    else:
                        print(f"  Current: {system_prompt[:80]}...\n")
...
def _run_serve(
    ...
    if engine in {"vllm", "agentic"} and model_path:
        try:
            ...
            def ui_generate_fn(message: str, history: list) -> str:
                request = ChatRequest(
                    model="aurelius",
                ...
                return api_generate_fn(request)

        except Exception as exc:
            if _RICH and _console:
                _console.print(
                    f"  [aurelius.warn]Warning:[/aurelius.warn] [dim]could not load {engine} engine ({exc}). Running in mock mode.[/dim]\n"  # noqa: E501
                )
            else:
                warning = f"  Warning: could not load {engine} engine ({exc})."
                print(YELLOW(f"{warning} Running in mock mode.\n"))
            api_generate_fn = None
            ui_generate_fn = None

    if api_generate_fn is None or ui_generate_fn is None:
        ...
        api_generate_fn = make_mock_generate_fn()
        ui_generate_fn = mk_ui()

    if _RICH and _console:
        _console.print(f"  [aurelius.system]Engine:[/aurelius.system] [dim]{backend_label}[/dim]")
    else:
        print(DIM(f"  Engine: {backend_label}"))

    try:
        api_server = create_server(host, port, api_generate_fn)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        examples:
          ...
          aurelius chat --react --model-path ckpt  enable ReAct mode
          ...
          aurelius serve --engine agentic   ReAct API backend
    ...
    chat_p.add_argument("--react", action="store_true", help="enable the ReAct tool-use loop")
    ...
    serve_p.add_argument(
        "--engine",
        choices=["mock", "vllm", "agentic"],
        default="mock",
        help="backend engine for the API server",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command is None or args.command == "chat":
        ...
        _run_serve(
            ...
            model_path=getattr(args, "model_path", None),
            engine=getattr(args, "engine", "mock"),
        )
        ...
        try:
            from aurelius_cli.family_commands import dispatch_family_command
        ...
        try:
            from aurelius_cli.backend_commands import dispatch_backend_command
        ...
        try:
            from aurelius_cli.session_commands import dispatch_session_command
        ...
        try:
            from aurelius_cli.interface_commands import dispatch_interface_command
...
def _run_config(args: argparse.Namespace) -> int:
    ...
    from aurelius_cli.config import CLIAppConfig
