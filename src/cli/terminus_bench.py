...
class TerminalBench:
    ...
    TASKS: list[TerminalBenchTask] = [
        TerminalBenchTask("tb-001", "Find all Python files larger than 1MB and list them sorted by size"),
    ...
    def run_task(self, task: TerminalBenchTask) -> dict[str, Any]:
        ...
        return {"task_id": task.id, "description": task.description, "result": result[:200] if result else ""}
