...
class AgentRouter:
    ...
    def classify(self, task: str) -> AgentMode:
        ...
        if any(
            kw in task_lower
            for kw in ["security", "vulnerability", "cve", "threat", "audit", "attack"]
        ):
            return self.modes["security"]
        if any(
            kw in task_lower for kw in ["debug", "bug", "fix", "error", "crash", "issue", "failing"]
        ):
            return self.modes["debug"]
        if any(
            kw in task_lower
            for kw in ["design", "architecture", "plan", "structure", "component", "system"]
        ):
            return self.modes["architect"]
        if any(
            kw in task_lower
            for kw in ["research", "analysis", "analyze", "understand", "explain", "document"]
        ):
