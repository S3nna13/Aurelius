...
class OpenAIChatRequestValidator:
    ...
    def validate(self, payload: dict) -> list[APIValidationError]:
        ...
        if "messages" not in payload:
            ...
            errors.append(
                APIValidationError("messages", "must contain at least one message", "empty_list")
            )
        else:
        ...
        if "temperature" in payload:
            ...
            if not _is_number(v):
                ...
                errors.append(
                    APIValidationError("temperature", "must be between 0 and 2", "out_of_range")
                )
        ...
        if "top_p" in payload:
            ...
            if not _is_number(v):
                ...
                errors.append(
                    APIValidationError("top_p", "must be between 0 and 1", "out_of_range")
                )
        ...
        if "max_tokens" in payload:
            ...
            if v is None:
                ...
                errors.append(
                    APIValidationError("max_tokens", "must be an integer", "invalid_type")
                )
            elif v < 1:
            ...
            if v is None or isinstance(v, str):
                ...
                for i, s in enumerate(v):
                    if not isinstance(s, str):
                        errors.append(
                            APIValidationError(f"stop[{i}]", "must be a string", "invalid_type")
                        )
                if len(v) > 4:
                    errors.append(
                        APIValidationError("stop", "at most 4 stop sequences", "out_of_range")
                    )
            else:
                errors.append(
                    APIValidationError(
                        "stop", "must be string, list of strings, or null", "invalid_type"
                    )
                )
        ...
        if "response_format" in payload:
            errors.extend(
                self._validate_response_format(payload["response_format"], "response_format")
            )
        ...
        if "top_logprobs" in payload:
            ...
            if not _is_int(v):
                errors.append(
                    APIValidationError("top_logprobs", "must be an integer", "invalid_type")
                )
            elif v < 0 or v > 20:
                errors.append(
                    APIValidationError("top_logprobs", "must be between 0 and 20", "out_of_range")
                )
        ...
        for key in ("presence_penalty", "frequency_penalty"):
            if key in payload:
                ...
                if not _is_number(v):
                    ...
                    errors.append(
                        APIValidationError(key, "must be between -2 and 2", "out_of_range")
                    )
        ...
        if "seed" in payload:
            ...
            if v is not None and not _is_int(v):
                errors.append(
                    APIValidationError("seed", "must be an integer or null", "invalid_type")
                )
        ...
        if self.strict:
            for key in payload:
                if key not in _REQUEST_KNOWN_FIELDS:
                    errors.append(
                        APIValidationError(
                            key, f"unknown field '{key}' (strict mode)", "unknown_field"
                        )
                    )
    ...
    def _validate_message(self, msg: Any, path: str) -> list[APIValidationError]:
        ...
        if has_content:
            ...
            if isinstance(content, str):
                ...
                errs.append(
                    APIValidationError(
                        f"{path}.content",
                        "must be a string or list of content parts",
                        "invalid_type",
                    )
                )
        ...
        if has_tool_calls:
            ...
            if not isinstance(tc, list):
                errs.append(
                    APIValidationError(f"{path}.tool_calls", "must be a list", "invalid_type")
                )
            else:
        ...
        if role == "tool" and "tool_call_id" not in msg:
            errs.append(
                APIValidationError(
                    f"{path}.tool_call_id", "required for role='tool'", "missing_field"
                )
            )
    ...
    def _validate_tool_call(self, call: Any, path: str) -> list[APIValidationError]:
        ...
        for key in ("id", "type", "function"):
            if key not in call:
                errs.append(
                    APIValidationError(f"{path}.{key}", "field is required", "missing_field")
                )
        ...
        if fn is not None:
            if not isinstance(fn, dict):
                errs.append(
                    APIValidationError(f"{path}.function", "must be an object", "invalid_type")
                )
            else:
                if "name" not in fn:
                    errs.append(
                        APIValidationError(
                            f"{path}.function.name", "field is required", "missing_field"
                        )
                    )
                elif not isinstance(fn["name"], str) or not fn["name"]:
                    errs.append(
                        APIValidationError(
                            f"{path}.function.name", "must be a non-empty string", "invalid_type"
                        )
                    )
    ...
    def _validate_tool(self, tool: Any, path: str) -> list[APIValidationError]:
        ...
        if "function" not in tool:
            errs.append(
                APIValidationError(f"{path}.function", "field is required", "missing_field")
            )
            return errs
        fn = tool["function"]
        if not isinstance(fn, dict):
        ...
        if "name" not in fn:
            errs.append(
                APIValidationError(f"{path}.function.name", "field is required", "missing_field")
            )
        elif not isinstance(fn["name"], str) or not fn["name"]:
            errs.append(
                APIValidationError(
        ...
        if "description" in fn and not isinstance(fn["description"], str):
            errs.append(
                APIValidationError(
                    f"{path}.function.description", "must be a string", "invalid_type"
                )
            )
    ...
    def _validate_tool_choice(self, v: Any, path: str) -> list[APIValidationError]:
        if isinstance(v, str):
            if v not in ("none", "auto", "required"):
                return [
                    APIValidationError(
                        path, "must be 'none', 'auto', or 'required'", "invalid_value"
                    )
                ]
            return []
        if isinstance(v, dict):
            ...
            if v.get("type") != "function":
                errs.append(
                    APIValidationError(f"{path}.type", "must be 'function'", "invalid_value")
                )
            fn = v.get("function")
            if not isinstance(fn, dict):
                errs.append(
    ...
    def _validate_response_format(self, v: Any, path: str) -> list[APIValidationError]:
        ...
        if rtype == "json_schema":
            if "json_schema" not in v:
                return [
                    APIValidationError(f"{path}.json_schema", "field is required", "missing_field")
                ]
            if not isinstance(v["json_schema"], dict):
                return [
                    APIValidationError(f"{path}.json_schema", "must be an object", "invalid_type")
                ]
        return []
...
class OpenAIChatResponseValidator:
    ...
    def validate(self, payload: dict) -> list[APIValidationError]:
        ...
        if "object" in payload and payload["object"] not in (
            "chat.completion",
            ...
            errors.append(
                APIValidationError(
                    "object",
                    "must be 'chat.completion' or 'chat.completion.chunk'",
                    "invalid_value",
                )
            )
        if "created" in payload and not _is_int(payload["created"]):
            errors.append(
                APIValidationError("created", "must be an integer unix timestamp", "invalid_type")
            )
    ...
    def _validate_choice(self, ch: Any, path: str, is_chunk: bool) -> list[APIValidationError]:
        ...
        if "index" not in ch:
            ...
            errs.append(
                APIValidationError(
                    f"{path}.index", "must be a non-negative integer", "invalid_type"
                )
            )
        ...
        if msg_key not in ch:
            errs.append(
                APIValidationError(f"{path}.{msg_key}", "field is required", "missing_field")
            )
        else:
            ...
            if fr not in _VALID_FINISH_REASONS:
                errs.append(
                    APIValidationError(
                        ...
                        f"must be one of {sorted(r for r in _VALID_FINISH_REASONS if r is not None)} or null",  # noqa: E501
                        "invalid_value",
                    )
                )
        elif not is_chunk:
            errs.append(
                APIValidationError(f"{path}.finish_reason", "field is required", "missing_field")
            )

        return errs

    def _validate_response_message(
        self, msg: Any, path: str, is_chunk: bool
        ...
        if not is_chunk:
            if "role" not in msg:
                errs.append(
                    APIValidationError(f"{path}.role", "field is required", "missing_field")
                )
            elif msg["role"] not in _VALID_MESSAGE_ROLES:
        ...
        if "content" in msg:
            ...
            if c is not None and not isinstance(c, str):
                errs.append(
                    APIValidationError(
                        f"{path}.content", "must be a string or null", "invalid_type"
                    )
                )
    ...
    def _validate_usage(self, u: Any, path: str) -> list[APIValidationError]:
        ...
        for key in ("prompt_tokens", "completion_tokens", "total_tokens"):
            if key in u and (not _is_int(u[key]) or u[key] < 0):
                errs.append(
                    APIValidationError(
                        f"{path}.{key}", "must be a non-negative integer", "invalid_type"
                    )
                )
        return errs
...
def _selftest() -> None:  # pragma: no cover - smoke
    ...
    assert v.validate({"model": "m", "messages": [{"role": "user", "content": "hi"}]}) == []  # noqa: S101
