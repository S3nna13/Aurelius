...
class CPUProfiler:
    ...
    def record(self, overall_pct: float, per_core_pct: list[float]) -> CPUSample:
        sample = CPUSample(
            timestamp=time.monotonic(),
            overall_pct=overall_pct,
            per_core_pct=list(per_core_pct),
            num_cores=len(per_core_pct),
        )
    ...
    def hottest_core(self) -> Optional[int]:
        ...
        if max_cores == 0:
            return None
        # Compute mean per-core across all samples
...
CPU_PROFILER_REGISTRY = {"default": CPUProfiler}
