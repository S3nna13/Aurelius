...
_INSTRUCTION_FILENAMES = ("AGENTS.md", "SOUL.md", "TOOLS.md", "IDENTITY.md", "USER.md")
_BOOTSTRAP_FILENAME = "BOOTSTRAP.md"
...
class SkillCatalog:
    ...
    def find_by_trigger(self, query: str) -> list[SkillCatalogEntry]:
        """Find skills whose 'Use when [condition]' description matches *query*.

        OpenClaw/Hermes-compatible trigger matching: the description field
        should follow the convention ``Use when [condition]``.  Skills are
        matched by checking if the description contains the *query* keyword
        and describes a scenario the agent can use.
        """
        if not isinstance(query, str) or not query.strip():
            raise InterfaceFrameworkError("query must be a non-empty string")
        needle = query.casefold()
        matches = []
        for entry in self.list(active=True):
            desc = entry.description.casefold()
            if needle in desc:
                matches.append(entry)
        return matches
    ...
    def _read_instruction_files(self, root: Path, *, label: str) -> list[str]:
        ...
        # Bootstrap file: one-time first-run ritual.
        bootstrap = root / _BOOTSTRAP_FILENAME
        if bootstrap.exists() and bootstrap.is_file():
            text = _read_text(bootstrap)
            layers.append(f"{label} bootstrap (one-time): {bootstrap}\n{text}")
