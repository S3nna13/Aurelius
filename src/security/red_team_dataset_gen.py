...
class RedTeamGenerator:
    ...
    def generate_dataset(
        ...
        if "prefix" in include_types:
            all_samples.extend(
                self.generate_prefix_injection(self.config.n_adversarial)
            )
        if "suffix" in include_types:
            all_samples.extend(
                self.generate_suffix_injection(self.config.n_adversarial)
            )
        if "noise" in include_types:
            all_samples.extend(
                self.generate_random_token_attack(self.config.n_adversarial)
            )
