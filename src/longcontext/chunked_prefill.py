...
class ChunkedPrefill:
    ...
    def __init__(self, config: ChunkedPrefillConfig) -> None:
        if not isinstance(config, ChunkedPrefillConfig):
            raise TypeError(
                "config must be a ChunkedPrefillConfig instance, got "
                f"{type(config).__name__}"
            )
        if config.chunk_size <= 0:
            raise ValueError(
                f"chunk_size must be > 0, got {config.chunk_size}"
            )
        if config.overlap < 0:
            raise ValueError(
                f"overlap must be >= 0, got {config.overlap}"
            )
    ...
    def iter_chunks(
        ...
        if input_ids.dim() < 2:
            raise ValueError(
                "input_ids must have at least 2 dims [B, S], got shape "
    ...
    def run_chunk_fn(
        ...
        for _, _, chunk in self.iter_chunks(input_ids):
            ...
            if not isinstance(out, torch.Tensor):
                raise TypeError(
                    "chunk_fn must return a torch.Tensor, got "
                    f"{type(out).__name__}"
                )
            outputs.append(out)
        if not outputs:
