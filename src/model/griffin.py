...
class RGLRULayer(nn.Module):
    ...
    def forward(
        ...
        state: Tensor | None = None,  # (B, d_model) or None
...
class GriffinModel(nn.Module):
    ...
    def __init__(self, config: GriffinConfig) -> None:
        ...
        self.blocks = nn.ModuleList([GriffinBlock(config, bt) for bt in block_types])
    ...
    def forward(
        ...
        states: list | None = None,  # list of per-block states, length n_layers
        ...
        if states is None:
            ...
            raise ValueError(f"Expected {n_blocks} states (one per block), got {len(states)}")
