...
class HuffmanTree:
    ...
    def build(self) -> None:
        ...
        heap: list[HuffmanNode] = [
            HuffmanNode(freq=freq_list[i], word_id=i) for i in range(self.vocab_size)
        ]
        heapq.heapify(heap)
    ...
    def _build_encoding(self, node: HuffmanNode | None) -> None:
        if node is None:
            return
        if node.word_id is not None:
    ...
    def _max_depth(self, node: HuffmanNode | None) -> int:
        if node is None:
            return 0
...
class HierarchicalSoftmax(nn.Module):
    ...
    def __init__(
        ...
        self.inner_weights = nn.Parameter(torch.empty(n_inner, d_model).normal_(0, 0.01))
    ...
    def sample(self, input: Tensor) -> Tensor:
        ...
        for b in range(B):
            ...
            while node is not None and node.word_id is None:
                ...
                # for either child)
                node.left.inner_nodes[0] if (
                    node.left is not None and node.left.inner_nodes
                ) else None
                node.right.inner_nodes[0] if (
                    node.right is not None and node.right.inner_nodes
                ) else None
