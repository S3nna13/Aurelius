...
class NTMHead(nn.Module):
    ...
    def _parse_params(
        ...
        offset = 0
        key   = raw[:, offset : offset + W];  offset += W
        beta  = raw[:, offset : offset + 1];  offset += 1
        gate  = raw[:, offset : offset + 1];  offset += 1
        shift = raw[:, offset : offset + 3];  offset += 3
        gamma = raw[:, offset : offset + 1];  offset += 1
        ...
        if self.head_type == "write":
            erase = raw[:, offset : offset + W];  offset += W
            add   = raw[:, offset : offset + W];  offset += W
...
class NTMModel(nn.Module):
    ...
    def __init__(
        ...
        self.read_heads  = nn.ModuleList([
            NTMHead(d_controller, word_size, n_locations, "read")
            for _ in range(n_reads)
        ])
        self.write_heads = nn.ModuleList([
            NTMHead(d_controller, word_size, n_locations, "write")
    ...
    def reset(self, batch_size: int) -> None:
        ...
        self._reads = [
            torch.zeros(batch_size, self.word_size, device=device)
