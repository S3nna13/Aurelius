...
class QueryComplexityEstimator:
    ...
    def extract_features(self, input_ids: List[int]) -> Dict[str, float]:
        ...
        clause_seps = sum(
            1 for t in input_ids
            if t in self._COMMA_IDS or t in self._SEMICOLON_IDS
        )
...
class CascadeRouter:
    ...
    def __init__(
        ...
        confidence_fn: Optional[Callable] = None,  # custom confidence scorer
    ) -> None:
        self.models = models
    ...
    def decode(
        ...
        decisions: List[Optional[RouteDecision]] = [None] * B
