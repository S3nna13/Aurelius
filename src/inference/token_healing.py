...
def greedy_extend(
    model_fn: Callable[[torch.Tensor], torch.Tensor], token_ids: torch.Tensor, n_steps: int
) -> torch.Tensor:
...
class TokenHealer:
    ...
    def __init__(
        ...
        config: TokenHealingConfig | None = None,
    ) -> None:
    ...
    def rollback_tokens(self, token_ids: torch.Tensor, n: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def get_continuation_candidates(
        self, prefix_ids: torch.Tensor, removed_ids: torch.Tensor, top_k: int
    ) -> torch.Tensor:
    ...
    def heal_and_continue(self, token_ids: torch.Tensor, n_new: int) -> torch.Tensor:
