    ...
    def quantize(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.per_channel and x.dim() >= 2:
            ...
            if self.symmetric:
                ...
                zero_points = (
                    (qmin - min_vals.squeeze(1) / scales).round().clamp(qmin, qmax).to(torch.int8)
                )
            x_reshaped = x.reshape(x.shape[0], -1)
            q = (
                (
                    x_reshaped / scales.unsqueeze(1).clamp(min=1e-8)
                    + zero_points.unsqueeze(1).to(x.dtype)
                )
                .round()
                .clamp(qmin, qmax)
                .to(torch.int8)
            ...
            if self.symmetric:
                ...
                if xmax == xmin:
                    ...
                    return (
                        torch.full_like(x, q_val, dtype=torch.int8),
