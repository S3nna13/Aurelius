...
class CodeReviewRubric:
    ...
    def __init__(self, weights: dict[ReviewDimension, float] | None = None) -> None:
        raw = weights if weights is not None else dict(self.DEFAULT_WEIGHTS)
        # Normalize to sum=1.0
        total = sum(raw.values())
        if total == 0:
            raise ValueError("Weights must sum to a positive number.")
        self.weights: dict[ReviewDimension, float] = {dim: w / total for dim, w in raw.items()}
    ...
    def weighted_score(self, scores: list[DimensionScore]) -> float:
        ...
        weight_sum = sum(self.weights.get(dim, 0.0) for dim in provided_dims)
        if weight_sum == 0:
            return 0.0
        total = sum(ds.score * self.weights.get(ds.dimension, 0.0) for ds in scores)
...
class CodeReviewScorer:
    def __init__(self, rubric: CodeReviewRubric | None = None) -> None:
    ...
    def score_review(self, dimension_scores: list[DimensionScore]) -> dict:
        ...
        missing = sorted(d.value for d in (all_dims - provided_dims))
