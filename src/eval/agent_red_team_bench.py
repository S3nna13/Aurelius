...
class ARTBenchmark:
    ...
    def __init__(
        ...
        attacker_profiles: tuple[AttackerProfile, ...] | None = None,
        ...
        if (
            ...
            or not all(isinstance(p, AttackerProfile) for p in attacker_profiles)
        ):
            raise TypeError("attacker_profiles must be a non-empty tuple of AttackerProfile")
        if (
            not isinstance(k_values, tuple)
            or not k_values
            or not all(isinstance(k, int) and k > 0 for k in k_values)
        ):
            raise ValueError("k_values must be a non-empty tuple of positive ints")
    ...
    def run_attacker(self, attacker_id: str, max_attempts: int) -> ARTResult:
        ...
        asr_at_k: dict[int, float] = {k: _asr_at_k(p_hat, k) for k in self.k_values}
        return ARTResult(
...
def stub_judge_fn(prompt: str, response: str) -> tuple[bool, float, str]:
    if not isinstance(prompt, str):
        raise TypeError("prompt must be str")
