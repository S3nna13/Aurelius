...
class CompletionSampler:
    ...
    def sample(self, logits: Tensor, n_samples: int) -> Tensor:
        ...
        # interleave so rows [0..K-1] are for batch item 0, etc.
        logits_rep = logits.repeat_interleave(n_samples, dim=0)  # (B*K, T, V)

        # Scale by temperature
        scaled = logits_rep / self.temperature  # (B*K, T, V)

        # Sample independently at each position; flatten to (B*K*T, V)
    ...
    def log_probs_of(self, logits: Tensor, token_ids: Tensor) -> Tensor:
        """Compute per-sequence mean log-probability.

        Averages the per-token log-probability over the sequence length T,
        with positions where token_ids == -1 treated as padding and excluded.
        ...
        Returns:
            Tensor of shape (B,) — mean log-probability over valid positions.
        """
        B, T, V = logits.shape
        ...
        # Padding mask: True where token is NOT padding
        pad_mask = token_ids != -1  # (B, T), bool

        # Clamp ids so gather is safe (padding positions map to 0, will be zeroed)
        safe_ids = token_ids.clone()
        safe_ids[~pad_mask] = 0

        # Gather log-probs of target tokens: (B, T)
...
class OnlinePairBuilder:
    ...
    to (B, K), finds argmax (chosen) and argmin (rejected) per prompt, and returns
    global indices into the flat vector.
    ...
    def build_pairs(
        ...
        Args:
            ...
            group_size: Number of completions per prompt (K). Defaults to
                        config.n_completions when None.
        ...
        chosen_local = rewards_2d.argmax(dim=1)  # (B,)
        rejected_local = rewards_2d.argmin(dim=1)  # (B,)

        # Convert local to global flat indices
...
class OnlineDPOLoss(nn.Module):
    ...
    def forward(
        ...
        """
        # Implicit reward under policy relative to reference
