...
class PRAXISTrainer:
    ...
    def __init__(
        ...
        self.bundle = RewardSignalBundle(
            config,
            self.prime,
            self.critique,
    ...
    def train_step(self, batch: dict, step: int) -> dict:
        ...
        # PrecisionFusion
        means = [
            signals[k][0] for k in ["r_prime", "r_const", "r_ccot", "r_odin", "r_hier", "r_src"]
        ]
        stds = [
            signals[k][1] for k in ["r_prime", "r_const", "r_ccot", "r_odin", "r_hier", "r_src"]
        ]
        ...
        if (
            step % cfg.warp_interval == 0
            and self.sft_state_dict
            and hasattr(self.model, "state_dict")
        ):
