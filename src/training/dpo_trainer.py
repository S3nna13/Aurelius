...
class SequenceLogProbs:
    ...
    def compute(
        ...
        token_logps = log_probs_all.gather(dim=-1, index=gather_labels.unsqueeze(-1)).squeeze(
            -1
        )  # (B, T-1)
...
class DPOTrainer:
    ...
    def train_step(
        ...
        ref_chosen_logps = self.ref_model_manager.compute_logps(chosen_ids, chosen_labels)
        ref_rejected_logps = self.ref_model_manager.compute_logps(rejected_ids, rejected_labels)
...
class PreferenceDataset(Dataset):
    ...
    def __init__(
        ...
        if len(chosen_ids_list) != len(rejected_ids_list):
            raise ValueError("chosen_ids_list and rejected_ids_list must have the same length")
