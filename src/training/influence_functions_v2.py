...
class LiSSAInfluence:
    ...
    def estimate(
        ...
        accum: Dict[str, Tensor] = {
            name: torch.zeros_like(test_grad[name])
        ...
        if not train_batches:
            ...
            return {name: test_grad.get(name, torch.zeros_like(accum[name])).clone()
                    for name in param_names}
        ...
        result: Dict[str, Tensor] = {
            name: accum[name] / self.n_samples for name in param_names
        }
        return result
