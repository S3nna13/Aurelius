...
import json, subprocess, tempfile, shutil, random
...
def msg(r,c): return {"role":r,"content":c}
def add(msgs): return {"messages": [msg("system",SYSTEM)] + msgs}
        ...
        for i, ex in enumerate(ds):
            ...
            try:
                if kind == "arxiv":
                    ...
                    if not a or len(a) < 50: continue
                    title = t[:200] if t else "paper"
                    f.write(json.dumps(add([msg("user",f"Summarize: {title}"),msg("assistant",a[:2000])]))+"\n")
                    count += 1
                elif kind == "reddit":
                    ...
                    f.write(json.dumps(add([msg("user",title),msg("assistant",str(a)[:2500])]))+"\n")
                    count += 1
                elif kind == "text":
                    t = (ex.get("text") or "").strip()
                    if len(t) < 300: continue
                    f.write(json.dumps(add([msg("user","Tell me about this."),msg("assistant",t[:2500])]))+"\n")
                    count += 1
                elif kind == "qa":
                    q = (ex.get(title_f) or "").strip() if title_f else ""
                    a = (ex.get(text_f) or "").strip() if text_f else ""
                    if not q: q = (ex.get("question") or "").strip()
                    if not a: a = (ex.get("long_answer") or ex.get("answer") or "").strip()
                    if not q or not a or len(a) < 50: continue
                    f.write(json.dumps(add([msg("user",q[:1000]),msg("assistant",a[:2500])]))+"\n")
                    count += 1
            except: pass
            if (i+1)%500==0: print(f"  {hf_id.split('/')[-1]}: {i+1}...",flush=True)
...
for r in range(1000):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        if out is None or not out.exists():
            print("  no data, skip", flush=True); continue
        merged = tmp / "m.jsonl"
