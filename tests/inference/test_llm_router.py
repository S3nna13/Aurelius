...
def test_complexity_long_vs_short():
    ...
    assert score_long > score_short, f"Expected long ({score_long:.4f}) > short ({score_short:.4f})"
...
def test_router_trainer_loss_decreases():
    ...
    assert last_loss < first_loss, f"Expected loss to decrease: {first_loss:.4f} -> {last_loss:.4f}"
