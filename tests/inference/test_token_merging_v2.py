...
def test_select_tokens_src_length():
    ...
    assert src.shape[0] == expected_k, (
        f"Expected {expected_k} src indices, got {src.shape[0]}"
    )
...
def test_select_tokens_keep_start_not_in_src():
    ...
    for idx in src.tolist():
        assert idx not in protected, (
            f"Protected token {idx} appeared in src_indices"
        )
...
def test_merge_tokens_output_shape():
    ...
    assert merged.shape == (T - k, d), (
        f"Expected shape ({T - k}, {d}), got {merged.shape}"
    )
...
def test_unmerge_tokens_output_shape():
    ...
    assert restored.shape == (T, d), (
        f"Expected shape ({T}, {d}), got {restored.shape}"
    )
