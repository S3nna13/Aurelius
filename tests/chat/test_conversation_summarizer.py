...
def test_extract_topics_excludes_stopwords():
    cs = ConversationSummarizer()
    result = cs.extract_topics(["the a an is in to of and or for with that this it be was are were has have"])
    assert all(w not in {"the", "a", "an", "is", "in", "to", "of", "and", "or", "for",
...
def test_summarize_extractive_compression_ratio_less_than_1():
    ...
    msgs = [f"This is a longer message number {i} with extra words to make it long enough" for i in range(20)]
    result = cs.summarize(msgs)
