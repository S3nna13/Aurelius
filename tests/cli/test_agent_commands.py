...
class TestAgentCLIDispatcher:
    def test_register_and_dispatch(self):
        ...
        cmd = AgentCLICommand(
            name="greet", description="say hello", handler=lambda args: f"hello {' '.join(args)}"
        )
    ...
    def test_alias_dispatches_same_handler(self):
        dispatcher = AgentCLIDispatcher()
        cmd = AgentCLICommand(
            name="help", description="show help", handler=lambda _: "help text", aliases=["h", "?"]
        )
        dispatcher.register(cmd)
    ...
    def test_list_commands_no_duplicates(self):
        dispatcher = AgentCLIDispatcher()
        cmd1 = AgentCLICommand(name="a", description="first", handler=lambda _: "a", aliases=["a1"])
        cmd2 = AgentCLICommand(name="b", description="second", handler=lambda _: "b")
