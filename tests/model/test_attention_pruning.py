...
def test_compute_head_importance_magnitude_shape():
    ...
    assert importance.shape == (N_HEADS,), f"Expected ({N_HEADS},), got {importance.shape}"
...
def test_compute_head_importance_random_deterministic():
    ...
    assert torch.allclose(imp_a, imp_b), "Random importances with the same seed must be identical"
...
def test_get_heads_to_prune_count():
    ...
    assert len(heads) == expected, f"Expected {expected} heads to prune, got {len(heads)}"
...
def test_apply_head_mask_zeros_pruned_heads():
    ...
    assert out[:, 1, :, :].abs().sum().item() == pytest.approx(0.0), "Head 1 should be zeroed"
    assert out[:, 3, :, :].abs().sum().item() == pytest.approx(0.0), "Head 3 should be zeroed"

    # Kept heads should be unchanged
    assert torch.allclose(out[:, 0, :, :], attn_output[:, 0, :, :]), "Head 0 should be unchanged"
    assert torch.allclose(out[:, 2, :, :], attn_output[:, 2, :, :]), "Head 2 should be unchanged"
...
def test_head_pruner_analyze_heads_shape():
    cfg = PruningConfig(
        n_heads=N_HEADS, d_head=D_HEAD, prune_ratio=PRUNE_RATIO, importance_metric="magnitude"
    )
