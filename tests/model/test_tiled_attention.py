...
def test_online_softmax_update_shapes():
    ...
    l = torch.zeros(B, H, tq, 1)
    O = torch.zeros(B, H, tq, D_HEAD)
    Q_i = torch.randn(B, H, tq, D_HEAD)
    K_b = torch.randn(B, H, tk, D_HEAD)
    V_b = torch.randn(B, H, tk, D_HEAD)

    m_new, l_new, O_new = OnlineSoftmax.update(m, l, O, Q_i, K_b, V_b, SCALE)
...
def test_online_softmax_l_positive():
    ...
    _, l_new, _ = OnlineSoftmax.update(m, l, O, Q_i, K_b, V_b, SCALE)
...
def test_online_softmax_single_block_matches_standard():
    ...
    l = torch.zeros(B, H, T, 1)
    O = torch.zeros(B, H, T, D_HEAD)

    _, _, O_online = OnlineSoftmax.update(m, l, O, Q_i, K_b, V_b, SCALE)
...
def test_tiled_attention_forward_shape():
    ...
    Q, K, V = _qkv()
    O = TiledAttention.apply(Q, K, V, SCALE, True, TILE)
    assert O.shape == (B, H, T, D_HEAD), f"Output shape {O.shape}"
...
def test_equivalence_checker_standard_attention_shape():
    ...
    Q, K, V = _qkv()
    O = _checker.standard_attention(Q, K, V, causal=True)
    assert O.shape == (B, H, T, D_HEAD), f"Shape {O.shape}"
