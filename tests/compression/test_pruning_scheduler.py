...
class TestPruningConfig:
    ...
    def test_custom(self):
        cfg = PruningConfig(
            initial_sparsity=0.1, target_sparsity=0.8, begin_step=50, end_step=500, frequency=50
        )
...
class TestSparsityAtBoundaries:
    def _sched(self, schedule: SparsitySchedule = SparsitySchedule.CUBIC) -> PruningScheduler:
        cfg = PruningConfig(
            initial_sparsity=0.0, target_sparsity=0.9, begin_step=100, end_step=1000
        )
...
class TestConstantSchedule:
    def _ps(self) -> PruningScheduler:
        cfg = PruningConfig(initial_sparsity=0.3, target_sparsity=0.9, begin_step=0, end_step=1000)
...
class TestLinearSchedule:
    def _ps(self) -> PruningScheduler:
        cfg = PruningConfig(initial_sparsity=0.0, target_sparsity=0.9, begin_step=0, end_step=1000)
...
class TestCubicSchedule:
    def _ps(
        ...
        cfg = PruningConfig(
            initial_sparsity=initial, target_sparsity=target, begin_step=begin, end_step=end
        )
    ...
    def test_cubic_faster_than_linear_early(self):
        ps_cubic = self._ps()
        cfg_lin = PruningConfig(
            initial_sparsity=0.0, target_sparsity=0.9, begin_step=0, end_step=1000
        )
