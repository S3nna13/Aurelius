...
def test_init_custom():
    q = QuantizationScheduler(
        initial_bits=32.0, final_bits=8.0, total_steps=500,
...
def test_linear_midpoint():
    q = QuantizationScheduler(
        initial_bits=16.0, final_bits=4.0, total_steps=1000,
...
def test_step_schedule_initial_plateau():
    q = QuantizationScheduler(
        schedule=QuantSchedule.STEP, total_steps=1000, step_interval=100,
    )
...
def test_step_schedule_decreases():
    q = QuantizationScheduler(
        initial_bits=16.0, final_bits=4.0, total_steps=1000,
        schedule=QuantSchedule.STEP, step_interval=100,
    )
    assert q.bits_at(1000) == pytest.approx(4.0)
