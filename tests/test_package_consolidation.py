...
def test_no_ambiguous_imports():
    ...
    assert gateway_path.endswith("gateway/__init__.py") or gateway_path.endswith("gateway/__init__.pyc"), \
...
def test_deprecation_warnings():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        import agent  # re-import fresh
        # Check for DeprecationWarning
        dep_warnings = [x for x in w if issubclass(x.category, DeprecationWarning)]
        assert len(dep_warnings) > 0, f"Expected DeprecationWarning for agent import, got {[str(x.message) for x in w]}"
