...
def test_higher_chosen_reward_lowers_loss():
    ...
    assert loss_boosted.item() < loss_base.item(), (
        f"Expected lower loss with boosted chosen; got {loss_boosted.item():.4f} vs {loss_base.item():.4f}"  # noqa: E501
    )
...
def test_metrics_keys():
    ...
    assert set(metrics.keys()) == expected_keys, f"Unexpected metrics keys: {set(metrics.keys())}"
...
def test_no_nan_with_extreme_log_probs():
    ...
    assert not torch.isnan(loss), "Loss is NaN with extreme inputs"
    assert not torch.isinf(loss), "Loss is Inf with extreme inputs"
...
def test_length_penalty_favours_concise_chosen():
    ...
    assert m2["reward_margin"] > 0, (
        f"Expected positive margin for higher per-token chosen reward; got {m2['reward_margin']:.6f}"  # noqa: E501
    )
