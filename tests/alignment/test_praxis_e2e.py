...
class FakeAureliusModel(nn.Module):
    ...
    def __init__(
        self, vocab_size: int = 128, d_model: int = 16, n_layers: int = 4, n_experts: int = 4
    ):
        super().__init__()
    ...
    def forward(
        self, input_ids, mask=None, labels=None, past_key_values=None, return_hidden_states=False
    ):
...
def make_trainer(cfg=None, n_layers=4):
    ...
    cfg = cfg or make_cfg()
    model = FakeAureliusModel(vocab_size=128, d_model=cfg.d_model, n_layers=n_layers, n_experts=4)
...
def test_e2e_warp_merge_fires_at_interval():
    ...
    merge_calls = []
    _original_anchor_merge = __import__(
        "src.alignment.warp", fromlist=["anchor_merge"]
    ).anchor_merge  # noqa: F841
...
def test_e2e_no_nan_signals():
    ...
    bundle = RewardSignalBundle(cfg, prime, critique, lambda x: torch.zeros(x.shape[0]), mc_reward)
