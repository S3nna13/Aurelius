...
def test_run_refinement_loop_returns_steps(model):
    ...
    trainer = _make_trainer(model, _make_reward_fn(0.5), n_refine_steps=3, stop_if_improved=False)
...
def test_run_refinement_loop_max_steps(model):
    ...
    n = 3
    trainer = _make_trainer(model, _make_reward_fn(0.5), n_refine_steps=n, stop_if_improved=False)
    steps = trainer.run_refinement_loop(
        prompt_ids=_ids(4),
...
def test_compute_refinement_gain_mean_improvement():
    ...
    steps = [SelfRefineStep(_ids(4), _ids(3), _ids(4), 0.0, 0.0 + v, v) for v in improvements]
    result = compute_refinement_gain(steps)
...
def test_generate_critique_and_refine_nonempty(trainer):
    ...
    critique_ids = trainer.generate_critique(prompt_ids, response_ids, critique_prompt_ids)
    assert critique_ids.ndim == 1
    assert critique_ids.numel() > 0, "critique_ids must be non-empty"

    refined_ids = trainer.refine(prompt_ids, response_ids, critique_ids, refine_prompt_ids)
