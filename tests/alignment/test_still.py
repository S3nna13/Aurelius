...
def test_filter_keeps_above_min_score():
    ...
    candidates = [
        [_make_sample(0.0, str(i)) for i in range(4)]
    ]
...
def test_filter_at_most_keep_top_k():
    ...
    candidates = [
        [_make_sample(1.0, str(i)) for i in range(4)]
    ]
    kept = trainer.filter_samples(candidates)
    assert len(kept) <= TINY_CFG.keep_top_k, (
        f"Expected <= {TINY_CFG.keep_top_k}, got {len(kept)}"
    )
...
def test_run_iteration_zero_scores_empty_kept():
    ...
    assert stats["n_kept"] == 0, (
        f"Expected n_kept=0 when all scores=0, got {stats['n_kept']}"
    )
