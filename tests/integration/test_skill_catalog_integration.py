...
def _write_skill(root: Path, *, skill_id: str, body: str, scope: str | None = None, frontmatter: bool = True) -> None:
...
def test_skill_catalog_discovery_precedence_and_round_trip(tmp_path):
    ...
    _write_skill(repo_root, skill_id="body-only", body="Body-only integration skill.", frontmatter=False)
    _write_skill(repo_root, skill_id="normalized-scope", body="Normalized scope body.", scope="workspace")
