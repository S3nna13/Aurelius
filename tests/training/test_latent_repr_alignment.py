...
def test_cosine_loss_identical_representations():
    ...
    aligner = RepresentationAligner(
        student_dim=64, teacher_dim=64, align_type="cosine", normalize=True
    )
    B, S, D = 2, 8, 64
    x = torch.randn(B, S, D)
...
def test_linear_projection_different_dims():
    ...
    aligner = RepresentationAligner(
        student_dim=student_dim, teacher_dim=teacher_dim, align_type="linear"
    )
...
def test_train_step_returns_required_keys():
    """train_step must return a dict containing 'loss', 'task_loss', 'alignment_loss', 'cosine_sim'."""  # noqa: E501
    student = _make_model(n_layers=2, d_model=64)
    teacher = _make_model(n_layers=2, d_model=64)
...
def test_gradient_flows_through_alignment_loss():
    ...
    torch.manual_seed(42)
    aligner = RepresentationAligner(
        student_dim=32, teacher_dim=32, align_type="linear", normalize=True
    )
