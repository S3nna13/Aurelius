...
from src.training.freeze import freeze_layers, unfreeze_layers, freeze_except, get_trainable_param_count

@pytest.fixture
def small_model():
    cfg = AureliusConfig(n_layers=4, d_model=64, n_heads=2, n_kv_heads=2,
...
def test_freeze_returns_correct_count(small_model):
    n = freeze_layers(small_model, ["layers.0.*"])
    frozen_in_layer0 = sum(1 for name, _ in small_model.named_parameters() if name.startswith("layers.0."))
