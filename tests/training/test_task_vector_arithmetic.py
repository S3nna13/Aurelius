...
def test_task_vector_apply_scale_zero_equals_base():
    ...
    for (name, p_base), (_, p_new) in zip(
        base.named_parameters(), new_model.named_parameters()
    ):
...
def test_sparsify_keeps_correct_fraction():
    ...
    nonzero_params = sum(
        (p != 0).sum().item() for p in sparse_tv.vector.values()
    )
...
def test_multi_task_compose_returns_module():
    ...
    assert isinstance(result, nn.Module), (
        f"Expected nn.Module, got {type(result)}"
    )
...
def test_interpolate_models_alpha_one_matches_model_a():
    ...
    for (name, p_a), (_, p_res) in zip(
        model_a.named_parameters(), result.named_parameters()
    ):
...
def test_interpolate_models_alpha_zero_matches_model_b():
    ...
    for (name, p_b), (_, p_res) in zip(
        model_b.named_parameters(), result.named_parameters()
    ):
