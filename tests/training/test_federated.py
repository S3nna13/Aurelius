...
def test_clip_gradients_dp_clips_norm():
    ...
    assert total_norm <= clip_norm + 1e-5, f"Expected grad norm <= {clip_norm}, got {total_norm}"
...
def test_fedavg_aggregate_returns_mean():
    ...
    for key in global_params:
        ...
        assert torch.allclose(result[key].float(), expected, atol=1e-5), f"Mean mismatch at {key}"
...
def test_fedavg_aggregate_weighted_upweights_client():
    ...
    assert dist_to_b < dist_to_a, "Weighted avg should be closer to the high-weight client"
...
def test_fedavg_aggregate_uniform_exact_mean():
    ...
    client_param_list = [
        {k: torch.full_like(v, val) for k, v in global_params.items()} for val in values
    ]
