...
def test_prefix_labels_masked():
    input_ids = torch.randint(0, 100, (2, 16))
    result = prepare_prefix_lm_batch(input_ids, cfg=PrefixLMConfig(mask_prefix_labels=True), prefix_len=6)
...
def test_suffix_labels_not_masked():
    ...
    result = prepare_prefix_lm_batch(input_ids, cfg=PrefixLMConfig(mask_prefix_labels=True), prefix_len=4)
