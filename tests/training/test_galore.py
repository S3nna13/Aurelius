...
from src.training.galore import GaLoreAdamW, GaLoreProjector
...
def _make_grad(m: int = M, n: int = N, seed: int = 0) -> torch.Tensor:
    torch.manual_seed(seed) and torch.randn(m, n)
...
def test_project_output_shape():
    ...
    assert G_tilde.shape == (RANK, N), f"Expected ({RANK}, {N}), got {G_tilde.shape}"
...
def test_unproject_restores_shape():
    ...
    assert delta_W.shape == G.shape, f"Expected {G.shape}, got {delta_W.shape}"
...
def test_projection_updates_only_at_gap():
    ...
    for _ in range(T_PROJ - 1):
        ...
        assert torch.allclose(proj.P_t, P_after_first), "P_t changed before T_proj steps elapsed"
...
def test_optimizer_step_no_crash():
    model = _tiny_model()
    optimizer = GaLoreAdamW(model.parameters(), lr=1e-3, rank=RANK, update_proj_gap=T_PROJ)
...
def test_optimizer_state_is_low_rank():
    ...
    assert m1.shape == (expected_r, N), f"m1 shape {m1.shape} != ({expected_r}, {N})"
    assert m2.shape == (expected_r, N), f"m2 shape {m2.shape} != ({expected_r}, {N})"
...
def test_rank_clamp_no_crash():
    ...
    assert G_tilde.shape[0] <= min(m, n), f"rank not clamped: G_tilde.shape = {G_tilde.shape}"
    delta_W = proj.unproject(G_tilde)
...
def test_numerical_stability_large_gradients():
    ...
    for name, p in model.named_parameters():
        assert torch.isfinite(p).all(), f"NaN/Inf in {name} after large-gradient step"
...
def test_svd_recomputation_changes_projection():
    ...
    (P_initial * P_new).sum().abs() / (P_initial.norm() * P_new.norm() + 1e-12)
