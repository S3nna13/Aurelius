...
def test_hook_capture(tiny_model: _TinyTransformer, input_ids: Tensor) -> None:
    ...
    assert hook.captured.shape == (BATCH, SEQ_LEN, D_MODEL), f"captured shape {hook.captured.shape}"
...
def test_steered_forward_shape(
    tiny_model: _TinyTransformer, concept_vec: Tensor, input_ids: Tensor
) -> None:
...
def test_steered_forward_alpha_zero(
    tiny_model: _TinyTransformer, concept_vec: Tensor, input_ids: Tensor
) -> None:
    ...
    assert torch.allclose(unsteered, steered, atol=1e-5), "alpha=0 steering must not change logits"
...
def test_logit_diff_sign_and_finite(
    tiny_model: _TinyTransformer, concept_vec: Tensor, input_ids: Tensor
) -> None:
    steerer = ActivationSteerer(tiny_model, layers=[0])
    ...
    assert diff_pos > diff_neg, f"expected diff_pos({diff_pos:.4f}) > diff_neg({diff_neg:.4f})"
...
def test_hooks_removed_after_steered_forward(
    tiny_model: _TinyTransformer, concept_vec: Tensor, input_ids: Tensor
) -> None:
    steerer = ActivationSteerer(tiny_model, layers=[0, 1])
    ...
    assert torch.allclose(logits_a, logits_b), "lingering hooks affected subsequent forward pass"
