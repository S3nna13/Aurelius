...
def test_predictive_entropy_one_hot_is_zero():
    ...
    assert torch.allclose(out, torch.zeros(B), atol=1e-5), f"One-hot entropy should be 0, got {out}"
...
def test_sequence_confidence_range():
    ...
    assert 0.0 < conf.item() <= 1.0 + 1e-7, f"Confidence should be in (0,1], got {conf.item()}"
