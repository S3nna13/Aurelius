...
class TinyAttention(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int) -> None:
        ...
        self.attn_weights: Optional[torch.Tensor] = None   # [B, H, T, T]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Causal mask
        mask = torch.triu(
            torch.ones(T_local, T_local, device=x.device), diagonal=1
        ).bool()
...
def test_generate_minimal_pair_differs_at_exactly_one_position(cfg_gen, input_ids):
    ...
    for b in range(B):
        ...
        assert diff_positions == [pos], (
            f"Batch {b}: expected diff at [{pos}], got {diff_positions}"
        )
...
def test_total_effect_is_zero_for_identical_tokens(cfg_est, input_ids):
    ...
    assert math.isclose(effect, 0.0, abs_tol=1e-6), (
        f"Expected 0.0, got {effect}"
    )
...
def test_attention_rollout_output_shape(attn_analyzer, input_ids):
    ...
    assert rollout.shape == (B, T, T), (
        f"Expected ({B}, {T}, {T}), got {rollout.shape}"
    )
...
def test_attention_rollout_rows_sum_to_one(attn_analyzer, input_ids):
    ...
    assert torch.allclose(row_sums, torch.ones_like(row_sums), atol=1e-5), (
        f"Row sums deviate from 1: min={row_sums.min().item():.6f}, "
        f"max={row_sums.max().item():.6f}"
    )
...
def test_top_k_predictions_returns_k_tokens_per_position(logit_lens, input_ids):
    ...
    for pos_preds in preds:
        assert len(pos_preds) == k, (
            f"Expected {k} predictions, got {len(pos_preds)}"
        )
...
def test_prediction_confidence_by_layer_returns_float_list(logit_lens, input_ids):
    ...
    if isinstance(final_logits, (tuple, list)):
        ...
        if candidate is None or (
            isinstance(candidate, torch.Tensor) and candidate.dim() == 0
        ):
