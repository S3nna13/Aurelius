...
def test_small_depth_run_bounded_node_count():
    probe = TreeOfAttacksProbe(max_depth=2, beam_width=2, branch_factor=2, success_threshold=0.99)
    result = probe.run(
        "objective x",
        _always_low_target,
        _always_low_judge,
...
def test_success_at_depth_one():
    probe = TreeOfAttacksProbe(max_depth=3, beam_width=2, branch_factor=2, success_threshold=0.8)
    result = probe.run(
        "obj",
        _always_low_target,
        _always_high_judge,
...
def test_beam_width_one_is_linear_at_each_depth():
    probe = TreeOfAttacksProbe(max_depth=3, beam_width=1, branch_factor=1, success_threshold=0.999)
    # Can't hit threshold >1 so full expansion
    result = probe.run("obj", _always_low_target, _always_low_judge, _identity_mutator)
    # 1 root + 1 at each depth = 1+3=4
    assert len(result.nodes) == 4


def test_branch_factor_one_linear():
    probe = TreeOfAttacksProbe(max_depth=3, beam_width=4, branch_factor=1, success_threshold=0.999)
    result = probe.run("obj", _always_low_target, _always_low_judge, _identity_mutator)
...
def test_pruning_discards_low_score_siblings():
    ...
    probe = TreeOfAttacksProbe(max_depth=1, beam_width=1, branch_factor=4, success_threshold=0.999)
    result = probe.run("obj", len_target, len_judge, _identity_mutator)
...
def test_empty_objective_rejected():
    ...
    with pytest.raises(ValueError):
        probe.run("", _always_low_target, _always_low_judge, _identity_mutator)
    with pytest.raises(ValueError):
        probe.run("   ", _always_low_target, _always_low_judge, _identity_mutator)
...
def test_non_string_objective_rejects():
    ...
    with pytest.raises(TypeError):
        probe.run(123, _always_low_target, _always_low_judge, _identity_mutator)


def test_deterministic_run_same_inputs_same_outputs():
    probe = TreeOfAttacksProbe(max_depth=2, beam_width=2, branch_factor=2, success_threshold=0.99)
    r1 = probe.run("obj", stub_target_fn, stub_judge_fn, heuristic_mutator)
    r2 = probe.run("obj", stub_target_fn, stub_judge_fn, heuristic_mutator)
...
def test_best_id_points_to_highest_score():
    probe = TreeOfAttacksProbe(max_depth=2, beam_width=2, branch_factor=2, success_threshold=0.999)
    result = probe.run("obj", stub_target_fn, stub_judge_fn, heuristic_mutator)
...
def test_unicode_objective_accepted():
    probe = TreeOfAttacksProbe(max_depth=1, beam_width=1, branch_factor=1, success_threshold=0.999)
    result = probe.run("目标 🎯 obj", _always_low_target, _always_low_judge, _identity_mutator)
