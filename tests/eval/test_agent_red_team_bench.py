...
def _mixed_profile(pid: str, n_unsafe: int, n_safe: int) -> AttackerProfile:
    prompts = [f"UNSAFE:{i}" for i in range(n_unsafe)] + [
        f"safe-{i}" for i in range(n_safe)
    ]
...
def test_asr_at_k_keys_match_k_values():
    ...
    bench = ARTBenchmark(
        stub_judge_fn, stub_target_fn, attacker_profiles=(p,)
    )
...
def test_run_all_returns_per_attacker():
    ...
    bench = ARTBenchmark(
        stub_judge_fn, stub_target_fn, attacker_profiles=(p1, p2)
    )
...
def test_judge_raising_is_recorded_as_failure():
    ...
    bench = ARTBenchmark(
        bad_judge, stub_target_fn, attacker_profiles=(p,)
    )
    result = bench.run_attacker("p", 4)
    assert result.total_attempts == 4
    assert result.wins == 0
...
def test_target_raising_is_recorded_as_failure():
    ...
    p = _all_unsafe_profile("p", 4)
    bench = ARTBenchmark(
        stub_judge_fn, bad_target, attacker_profiles=(p,)
    )
    result = bench.run_attacker("p", 4)
    assert result.total_attempts == 4
    assert result.wins == 0
...
def test_determinism_same_inputs_same_outputs():
    ...
    bench_a = ARTBenchmark(
        stub_judge_fn, stub_target_fn, attacker_profiles=(p,)
    )
    bench_b = ARTBenchmark(
        stub_judge_fn, stub_target_fn, attacker_profiles=(p,)
    )
