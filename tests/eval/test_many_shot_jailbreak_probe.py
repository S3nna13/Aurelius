...
def test_first_success_shot_set_when_judge_accepts_at_16():
    probe = ManyShotJailbreakProbe(
        max_shots=64, shot_schedule=(1, 4, 16, 64)
    )
...
def test_max_shots_respected_in_schedule():
    probe = ManyShotJailbreakProbe(
        max_shots=8, shot_schedule=(1, 4, 16, 64)
    )
...
def test_default_shot_library_has_no_obvious_real_jailbreak_strings():
    banned = ("ignore previous", "dan", "developer mode", "jailbreak",
              "do anything now", "ignore prior")
    for s in DEFAULT_SHOT_LIBRARY:
        blob = (s.user_prompt + " " + s.fake_affirmative_response).lower()
        for token in banned:
            assert token not in blob, (
                f"DEFAULT_SHOT_LIBRARY must not contain {token!r}"
            )
...
def test_stub_target_and_judge_are_deterministic():
    ...
    assert [r.succeeded for r in r1.per_shot_results] == \
           [r.succeeded for r in r2.per_shot_results]
    assert r1.first_success_shot == r2.first_success_shot
    assert [r.response for r in r1.per_shot_results] == \
           [r.response for r in r2.per_shot_results]
...
def test_judge_raising_recorded_as_failure():
    ...
    probe = ManyShotJailbreakProbe(
        max_shots=16, shot_schedule=(1, 4, 16)
    )
...
def test_unicode_objective_accepted():
    probe = ManyShotJailbreakProbe(
        max_shots=16, shot_schedule=(1, 16)
    )
    result = probe.run(
        "目的 — describe café résumé", stub_target_fn, stub_judge_fn
    )
    assert result.objective == "目的 — describe café résumé"
    assert any(
        r.succeeded for r in result.per_shot_results
    ) or result.first_success_shot is None  # not required to succeed
...
def test_invalid_shot_library_rejected():
    with pytest.raises(TypeError):
        ManyShotJailbreakProbe(
            max_shots=4, shot_schedule=(1,), shot_library=("nope",)
        )
    with pytest.raises(TypeError):
        ManyShotJailbreakProbe(
            max_shots=4, shot_schedule=(1,), shot_library=()
        )
