...
def test_find_diagonal_heads_identity_all_diagonal():
    ...
    assert mask.all(), (
        f"Identity attention should mark all heads as diagonal, got {mask}"
    )
...
def test_find_previous_token_heads_pure_prev_all_true():
    ...
    assert mask.all(), (
        f"Pure previous-token attention should flag all heads, got {mask}"
    )
...
def test_attention_analyzer_analyze_layer_shapes():
    ...
    assert result["mean_distance"].shape == (H, T), f"mean_distance shape {result['mean_distance'].shape}"
    assert result["diagonal_heads"].shape   == (H,), f"diagonal_heads shape {result['diagonal_heads'].shape}"
    assert result["prev_token_heads"].shape == (H,), f"prev_token_heads shape {result['prev_token_heads'].shape}"
...
def test_attention_analyzer_summarize_keys_and_scalar():
    ...
    required_scalar_keys = {
        "mean_entropy", "mean_distance", "n_diagonal", "n_prev_token", "max_sink_score"
    }
...
def test_attention_analyzer_summarize_n_diagonal_nonneg():
    ...
    analysis = analyzer.analyze_layer(attn)
    summary  = analyzer.summarize(analysis)
    assert summary["n_diagonal"] >= 0, (
        f"n_diagonal must be >= 0, got {summary['n_diagonal']}"
    )
    assert summary["n_diagonal"] <= H, (
        f"n_diagonal must be <= H={H}, got {summary['n_diagonal']}"
    )
