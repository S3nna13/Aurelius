...
def test_js_import_regex_no_redos():
    ...
    assert elapsed < 1.0, (
        f"_JS_IMPORT_RE took {elapsed:.3f}s on adversarial input (ReDoS suspected)"
    )
...
def test_go_import_regex_no_redos():
    ...
    elapsed = time.perf_counter() - start
    assert elapsed < 1.0, (
        f"_GO_IMPORT_RE took {elapsed:.3f}s on adversarial input (ReDoS suspected)"
    )
...
def test_sse_tool_handler_exception_not_leaked():
    ...
    assert error_msg == "handler error", f"Expected generic 'handler error', got: {error_msg!r}"
