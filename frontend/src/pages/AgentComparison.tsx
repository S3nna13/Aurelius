...
import {
  Bot, Activity, HeartPulse, Zap, Pause, Play, ArrowLeft, BarChart3,
} from 'lucide-react';
...
function parseMetrics(metricsJson: string): Record<string, number> {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/')} className="p-2 rounded-lg text-aurelius-muted hover:text-aurelius-text hover:bg-aurelius-border/40 transition-colors">
          <ArrowLeft size={18} />
        </button>
      ...
      <p className="text-sm text-aurelius-muted">Select up to 3 agents to compare side-by-side.</p>
              ...
              <div key={agent.id} className="aurelius-card space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between py-2 border-b border-aurelius-border/50">
                    <span className="text-xs text-aurelius-muted flex items-center gap-1"><Activity size={12} /> State</span>
                    <span className={`text-xs font-bold ${isActive ? 'text-emerald-400' : 'text-amber-400'}`}>{agent.state}</span>
                  </div>
                  <div className="flex items-center justify-between py-2 border-b border-aurelius-border/50">
                    <span className="text-xs text-aurelius-muted flex items-center gap-1"><HeartPulse size={12} /> Uptime</span>
                    <span className="text-xs font-bold text-aurelius-text">{metrics.uptime_pct?.toFixed(1) ?? '--'}%</span>
                  </div>
                  <div className="flex items-center justify-between py-2 border-b border-aurelius-border/50">
                    <span className="text-xs text-aurelius-muted flex items-center gap-1"><Zap size={12} /> Latency</span>
                    <span className="text-xs font-bold text-aurelius-text">{metrics.latency_ms ?? '--'} ms</span>
                  </div>
                ...
                <button onClick={() => navigate(`/agents/${agent.id}`)} className="w-full aurelius-btn-outline text-xs">View Details</button>
              </div>
            );
          })}
        </div>
