...
class AureliusBlock14B(nn.Module):
    def __init__(self, d_model: int, n_heads: int, d_ff: int, d_mem: int,
                 ...
                 layer_idx: int = 0, agent_interval: int = 4):
        super().__init__()
...
class BrainBridge(nn.Module):
    def __init__(self, d_model: int, d_brain: int = 1536, n_actions: int = 8):
        super().__init__()
        self.proj_in = nn.Linear(d_model, d_brain)
        self.proj_out = nn.Linear(d_brain, d_model)
        self.gate = nn.Parameter(torch.zeros(1))
        self.planner = PlanningModule(d_brain, n_simulations=16, max_depth=6)
...
class AureliusModel14B(nn.Module):
    def __init__(self, config: dict):
        ...
        d_brain = config.get('d_brain', 1536)
        ...
        self.blocks = nn.ModuleList([
            AureliusBlock14B(
                ...
                agent_interval=config.get('agent_interval', 4),
        ...
        self.replay_buffer = ExperienceReplayBuffer(
            capacity=config.get('replay_capacity', 200000),
        )
    ...
    @classmethod
    def from_config_file(cls, path: str) -> 'AureliusModel14B':
        import yaml
        with open(path) as f:
