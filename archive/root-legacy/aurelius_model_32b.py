...
class AureliusBlock32B(nn.Module):
    def __init__(self, d_model: int, n_heads: int, d_ff: int, d_mem: int,
                 episodic_slots: int, lts_capacity: int,
                 skill_dim: int = 1024, n_known_tools: int = 512,
                 layer_idx: int = 0, agent_interval: int = 5):
        super().__init__()
        ...
        self.memory = AurelianMemoryCore(
            ...
            consolidation_freq=2048,
        )

        self.has_agent = (layer_idx % agent_interval == agent_interval - 1)
        if self.has_agent:
...
class BrainBridge(nn.Module):
    def __init__(self, d_model: int, d_brain: int = 2048, n_actions: int = 12):
        super().__init__()
        self.proj_in = nn.Linear(d_model, d_brain)
        self.proj_out = nn.Linear(d_brain, d_model)
        self.gate = nn.Parameter(torch.zeros(1))
        self.planner = PlanningModule(d_brain, n_simulations=32, max_depth=8)
...
class AureliusModel32B(nn.Module):
    def __init__(self, config: dict):
        ...
        skill_dim = config.get('skill_dim', 1024)
        n_known_tools = config.get('n_known_tools', 512)
        d_brain = config.get('d_brain', 2048)
        n_layers = config['n_layers']

        self.token_embedding = nn.Embedding(config['vocab_size'], d_model)
        self.rotary = RotaryEmbedding(d_model // n_heads, max_position=config['max_seq_len'])

        self.blocks = nn.ModuleList([
            AureliusBlock32B(
                d_model=d_model, n_heads=n_heads, d_ff=d_ff,
                d_mem=d_mem, episodic_slots=episodic_slots,
                lts_capacity=lts_capacity, skill_dim=skill_dim,
                n_known_tools=n_known_tools, layer_idx=i,
                agent_interval=config.get('agent_interval', 5),
        ...
        self.replay_buffer = ExperienceReplayBuffer(
            capacity=config.get('replay_capacity', 500000),
        )
    ...
    def _init_weights(self):
        for name, p in self.named_parameters():
            if p.ndim >= 2 and all(k not in name for k in ('memory', 'skill', 'tool', 'brain')):
    ...
    @classmethod
    def from_config_file(cls, path: str) -> 'AureliusModel32B':
        import yaml
        with open(path) as f:
...
sys.modules.setdefault("aurelius_model_32b", _module)
sys.modules.setdefault("aurelius.aurelius_model_32b", _module)
